...
## Three times the original size, at the owner's request — big enough to
## read clearly against a crowd of toy knights rather than disappear into it.
const CRATE_SIZE := 4.8
const CANOPY_RADIUS := 7.8
...
## event finishes touching it. The canopy is — a parachute still attached to
## a crate sitting in the grass reads as a mistake, not as a prop.
...
func _build() -> void:
	...
	crate.mesh = box
	# BoxMesh is centred on its own origin; left at (0, 0, 0) half the crate
	# would sit under the ground the node's own position already sits on.
	# Lifting it by half its height puts the bottom face on the ground
	# instead of the middle.
	crate.position = Vector3(0.0, CRATE_SIZE * 0.5, 0.0)
	...
	cloth.roughness = 1.0
	_canopy.material_override = cloth
	# Squashed rather than a full sphere, so it reads as a canopy rather than
	# a balloon tied to the crate.
	_canopy.scale = Vector3(1.0, 0.45, 1.0)
	_canopy.position = Vector3(0.0, CRATE_SIZE * 1.8, 0.0)
	add_child(_canopy)
