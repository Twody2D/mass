...
func _ready() -> void:
	...
	print("--- registration ---")
	failures += _check("free, orbit, fpv_drone and approach are registered by default",
		rig.known_modes() ==
			([&"free", &"orbit", &"fpv_drone", &"approach"] as Array[StringName]))
	...
	failures += _check("known modes list all six in order",
		rig.known_modes() ==
			([&"free", &"orbit", &"fpv_drone", &"approach", &"stub_a", &"stub_b"] as Array[StringName]))
